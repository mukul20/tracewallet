...
class BscscanTransformer
{
    ...
    static function transformTransactionsResponse(array $response, array $config): array
    {
        foreach ($response['result'] as $key => $transaction) {
            $response['result'][$key]['value'] = 
                self::convertToBNB($transaction['value']) . ' ' . $config['currency'];

            $response['result'][$key]['txFees'] = 
                self::convertToBNB($transaction['gasPrice'] * $transaction['gasUsed'])
                . ' ' . $config['currency'];
        }
    ...
    */
    ...
    {
        $response['result'] = self::convertToBNB($response['result'])
                                . ' ' . $config['currency'];
    ...
    /**
    * Convert string to BNB
    *
    * @param string $value  BNB token value
    * 
    * @return string;
    */
    static function convertToBNB(string $value): string
    {
        $decimalPlaces = 18;
